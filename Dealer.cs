...
namespace Waymo_car_dealership
{
    ...
    class Dealer
    {
        ...
        Int32[] diff = new Int32[] {75,100,200,300,400};                             // Maximum difference between the previous price and the current price to place order
            ...
            { 
                ...
                if (need > 0 && Math.Abs(Unit_Price - previous_cut) < diff[ID])   // Place an order if number of cars required by a dealer is greater than one and Maximum difference between the previous price and the current price is within limit
                {
                    ...
                    try
                    {
                        ...
                        Console.WriteLine("Dealer " + (ID+1) + " has placed the order -->  " + "Number Of Cars:" + dealer_object.getAmount() + "  UnitPrice:" + dealer_object.getUnitPrice() + " $k"+ "  CreditCard:" + dealer_object.getCreditCard());
